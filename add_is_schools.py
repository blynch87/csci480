...
url = f"https://meteor.unca.edu/registrar/transfer-equivalencies/api/v1/equivalencies/in-state-schools"
...
for school in data:
    cur.execute(
        "INSERT INTO schools (name, type, state_id, school_code) VALUES (%s, %s, %s, %s) ON CONFLICT (school_code) DO NOTHING;", #Make sure no duplicates, 
            (school["Name"], "in-state", 51, school["Code"])
    )
conn.commit()
print(f"Inserted {len(data)} schools for in-state schools")


cur.close()
conn.close()
print("All in-state schools imported successfully.")
